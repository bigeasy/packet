...
class module.exports.SerializerTest extends TwerpTest
  ...
  'test: write a zero terminated UTF-8 string after skipping': (done) ->
    ...
    serializer.buffer "x16{0}, b8z|utf8()", "ABC", (buffer) =>
      @equal serializer.getBytesWritten(), 6
      @deepEqual toArray(buffer), [ 0x00, 0x00, 0x41, 0x42, 0x43, 0x00 ]
      done 2
  ...
  'test: write a bit packed integer': (done) ->
    serializer = new Serializer()
    serializer.buffer "b8{x2,b3,x3}", 5, (buffer) =>
      @equal serializer.getBytesWritten(), 1
  ...
  'test: write a bit packed signed negative integer': (done) ->
    ...
    serializer.buffer "b8{x2,-b3,x3}", -4, (buffer) =>
      @equal serializer.getBytesWritten(), 1
      @deepEqual toArray(buffer), [ 0x20 ]
      done 2

  'test: write a bit packed signed integer': (done) ->
    serializer = new Serializer()
