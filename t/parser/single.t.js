...
require('./proof')(60, function (parseEqual) {
    ...
    parseEqual({ require: true, buffersOnly: true }, 'b8,b32f', new Buffer([ 0xcd, 0xcc, 0x2c, 0x41, 0x01 ].reverse()), 5, 1, 10.800000190734863, 'big positive, buffers only')
    parseEqual({ require: true, buffersOnly: true, subsequent: true }, 'b8,b32f', new Buffer([ 0xcd, 0xcc, 0x2c, 0x41, 0x01 ].reverse()), 5, 1, 10.800000190734863, 'big positive, buffers only, subsequent')
    parseEqual({ require: true, buffersOnly: true }, 'b8,b32f', new Buffer([ 0xcd, 0xcc, 0x2c, 0x41, 0x01 ].reverse()), [ 0, 5 ], 1, 10.800000190734863, 'big positive, buffers only, fallback')
    // Two in a row.
