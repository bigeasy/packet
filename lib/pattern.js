...
function compileRegularExpressions() {
  ...
  for (i = 0, I = source.length; i < I; i++, $ = null) {
    ...
    if ($) {
      ...
      re[name] = new RegExp(lines.join(""), $[1] || "");
    }
  }
}
...
function alternates (pattern, array, rest, primary, secondary, allowSecondary, index) {
  ...
  while (rest) {
    ...
    if ($ = /^(\s*)([^|]+)(\|\s*)(.*)$/.exec(rest)) {
      padding = $[1], part = $[2], delimiter = $[3], rest = $[4];
    } else {
      padding = "", part = rest, delimiter = "", rest = null;
    }
  ...
  for (;;) {
    ...
    if ($ = /^{((?:-b|b|x).+)}(\s*,.*|\s*)$/.exec(rest)) {
      ...
      */]);
      ...
      // they are packed.
      var sum = f.packing.reduce(function (x, y) { return x + y }, 0);
    ...
    // Neither bit packing nor alternation.
      ...
      if ($ = /^({\s*)((0x|0)?([a-f\d]+)\s*})(.*)$/i.exec(rest)) {
        before = $[1], after = $[2], based = $[3], pad = $[4], rest = $[5];
        index += before.length;
        ...
        if ($[1] != null) {
          ...
          for (;;) {
            ...
            */].exec(terminator);
            ...
            before = $[1], hex = $[2], decimal = $[3], comma = $[4], rest = $[5];
            index += before.length;
            numberIndex = index;
            if (hex) {
              value = parseInt(hex, 16);
